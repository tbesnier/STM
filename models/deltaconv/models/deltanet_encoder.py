...
class DeltaNetEncoder(torch.nn.Module):
    def __init__(self, in_channels, latent_size, conv_channels=[64, 64, 128, 256], num_neighbors=20, grad_regularizer=1e-3, grad_kernel_width=1):
        ...
        self.classification_head = Seq(
            ...
            Linear(256, latent_size))


    def forward(self, x):
        B, N, _ = x.shape[0], x.shape[1], x.shape[2]
        conv_out = self.deltanet_base(x)
