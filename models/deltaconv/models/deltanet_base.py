...
from ..geometry.operators import hodge_laplacian
    

class DeltaNetBase(torch.nn.Module):
    def __init__(self, in_channels, conv_channels, mlp_depth, num_neighbors, grad_regularizer, grad_kernel_width, centralize_first=False):
    ...
    def forward(self, x):
        ...
        pos = x.view(-1, 3)
        batch = torch.repeat_interleave(torch.arange(B), N).to("cuda:0")
        #batch = None #data.batch
        ...
        # if hasattr(x, 'norm') and data.norm is not None:
        #     normal = data.norm
        ...
        x = pos #data.x if hasattr(data, 'x') and data.x is not None else pos
