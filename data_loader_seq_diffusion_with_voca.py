...
def read_data(args, flag=None):
    ...
    # VOCA
    # ----------------------------
    templates_dir_voca = ensure_dir(args.templates_dir_VOCA, "templates_dir_VOCA")
    deformations_dir_voca = ensure_dir(args.deformations_dir_VOCA, "deformations_dir_VOCA")

    # np.load accepts Path; keep it as Path for correctness
    lmk_idx = np.load(as_path("./data/lmk_noeyes_idx.npy"))

    template_files_voca = iter_template_meshes(templates_dir_voca, exts)

    for tmpl_path in template_files_voca:
        template_name = tmpl_path.stem
        ...
        seq_dirs = find_matching_sequence_dirs(deformations_dir_voca, template_name)
        for seq_dir in seq_dirs:
            frame_files = iter_mesh_files(seq_dir, exts)
            print(f"  Sequence dir: {seq_dir.name}  ({len(frame_files)} frames)")
